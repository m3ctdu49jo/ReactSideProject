...
interface DetailDataItemsProps {
    id: number;
    orderNum: string;
    orderSeq: string;
    productId: string;
...
export default function GridDetailTest(){
    ...
    const [clickItem, setClickItem] = useState<dataItemsProps | undefined>(undefined);
    const [showDetail, setShowDetail] = useState(false);
    const [deatilData, setDetailData] = useState<DetailDataItemsProps[] | null>(null);
    const [isDetailLoading, setIsDetailLoading] = useState<boolean>(true);
    const [detailColNameList, setDetailColNameList] = useState<columnsNameProps[] | null>(null);
    ...
    useEffect(() => {
        async function fetchData() {              
            const t: dataProps<DetailDataItemsProps> = await new Promise((resolve) => {
                const filterD = data?.filter(x => x.orderNum === clickItem?.orderNum);
                let details: DetailDataItemsProps[] = [];
                filterD?.map(item => {
                    details.push({
                        id: item.id,
                        ...
                        price: item.id * 3
                    } as DetailDataItemsProps)
                });
    
                let detailColsName: columnsNameProps[] = [];
                detailColsName.push({ colId: "id", colName: "id", visible: false });
                detailColsName.push({ colId: "orderNum", colName: "訂單單號" });
                detailColsName.push({ colId: "orderSeq", colName: "訂單項次" });
                detailColsName.push({ colId: "productId", colName: "產品編號" });
                detailColsName.push({ colId: "productName", colName: "產品名稱" });
                detailColsName.push({ colId: "price", colName: "價格" });

                resolve({d:details, c:detailColsName});
            })
            setDetailData(t.d);
            setDetailColNameList(t.c);
            if(clickItem)
                setShowDetail(true);
    ...
    useEffect(() => {
        async function fetchData(){
            const t: dataProps<dataItemsProps> = await new Promise((resolve) => {             
